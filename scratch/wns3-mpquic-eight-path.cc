...
void ThroughputMonitor (FlowMonitorHelper *fmhelper, Ptr<FlowMonitor> flowMon, Ptr<OutputStreamWrapper> stream)
{
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator stats = flowStats.begin (); stats != flowStats.end (); ++stats)
    {
        ...
        if (stats->first == 1 || (stats->first >= 3 && stats->first <= 9)){
...
//  LogComponentEnable ("PacketMetadata", log_precision);
    ...
    NodeContainer n4n10 = NodeContainer (c.Get (4), c.Get (10));
    NodeContainer n10n11 = NodeContainer (c.Get (10), c.Get (11));
    NodeContainer n11n5 = NodeContainer (c.Get (11), c.Get (5));

    NodeContainer n4n12 = NodeContainer (c.Get (4), c.Get (12));
    NodeContainer n12n13 = NodeContainer (c.Get (12), c.Get (13));
    NodeContainer n13n5 = NodeContainer (c.Get (13), c.Get (5));

    NodeContainer n4n14 = NodeContainer (c.Get (4), c.Get (14));
    NodeContainer n14n15 = NodeContainer (c.Get (14), c.Get (15));
    NodeContainer n15n5 = NodeContainer (c.Get (15), c.Get (5));

    NodeContainer n4n16 = NodeContainer (c.Get (4), c.Get (16));
    NodeContainer n16n17 = NodeContainer (c.Get (16), c.Get (17));
    NodeContainer n17n5 = NodeContainer (c.Get (17), c.Get (5));
    ...
    internet.Install (c.Get (10));
    internet.Install (c.Get (11));
    internet.Install (c.Get (12));
    internet.Install (c.Get (13));
    internet.Install (c.Get (14));
    internet.Install (c.Get (15));
    internet.Install (c.Get (16));
    internet.Install (c.Get (17));
    ...
    d10d11.Get (1)->SetAttribute ("ReceiveErrorModel", PointerValue (em));
    NetDeviceContainer d12d13 = p2p.Install (n12n13);
    d12d13.Get (1)->SetAttribute ("ReceiveErrorModel", PointerValue (em));
    ...
    d14d15.Get (1)->SetAttribute ("ReceiveErrorModel", PointerValue (em));
    NetDeviceContainer d16d17 = p2p.Install (n16n17);
    d16d17.Get (1)->SetAttribute ("ReceiveErrorModel", PointerValue (em));
    ...
    NetDeviceContainer d4d12 = p2p.Install (n4n12);
    NetDeviceContainer d4d14 = p2p.Install (n4n14);
    ...
    NetDeviceContainer d11d5 = p2p.Install (n11n5);
    NetDeviceContainer d13d5 = p2p.Install (n13n5);
    NetDeviceContainer d15d5 = p2p.Install (n15n5);
    NetDeviceContainer d17d5 = p2p.Install (n17n5);
    ...
    Ipv4InterfaceContainer i4i12 = ipv4.Assign (d4d12);

    ipv4.SetBase ("10.1.15.0", "255.255.255.0");
    Ipv4InterfaceContainer i4i14 = ipv4.Assign (d4d14);

    ipv4.SetBase ("10.1.16.0", "255.255.255.0");
    Ipv4InterfaceContainer i4i16 = ipv4.Assign (d4d16);


    ipv4.SetBase ("10.1.17.0", "255.255.255.0");
    Ipv4InterfaceContainer i11i5 = ipv4.Assign (d11d5);

    ipv4.SetBase ("10.1.18.0", "255.255.255.0");
    Ipv4InterfaceContainer i13i5 = ipv4.Assign (d13d5);

    ipv4.SetBase ("10.1.19.0", "255.255.255.0");
    Ipv4InterfaceContainer i15i5 = ipv4.Assign (d15d5);

    ipv4.SetBase ("10.1.20.0", "255.255.255.0");
    Ipv4InterfaceContainer i17i5 = ipv4.Assign (d17d5);
    ...
    staticRouting_n4->AddHostRouteTo (Ipv4Address ("10.1.21.2"), Ipv4Address ("10.1.17.2") ,5); 
    staticRouting_n4->AddHostRouteTo (Ipv4Address ("10.1.22.2"), Ipv4Address ("10.1.18.2") ,6); 
    staticRouting_n4->AddHostRouteTo (Ipv4Address ("10.1.23.2"), Ipv4Address ("10.1.19.2") ,7); 
    staticRouting_n4->AddHostRouteTo (Ipv4Address ("10.1.24.2"), Ipv4Address ("10.1.20.2") ,8); 
    ...
    staticRouting_n5->AddHostRouteTo (Ipv4Address ("10.1.13.1"), Ipv4Address ("10.1.21.1") ,5); 
    staticRouting_n5->AddHostRouteTo (Ipv4Address ("10.1.14.1"), Ipv4Address ("10.1.22.1") ,6); 
    staticRouting_n5->AddHostRouteTo (Ipv4Address ("10.1.15.1"), Ipv4Address ("10.1.23.1") ,7); 
    staticRouting_n5->AddHostRouteTo (Ipv4Address ("10.1.16.1"), Ipv4Address ("10.1.24.1") ,8); 
    ...
    // Simulator::Schedule (Seconds (start_time+0.0000001), &Traces, c.Get (4)->GetId(), file.str (), ".txt");
    ...
    for (double i = 1; i < simulationEndTime; i = i+0.1){
        ...
        Simulator::Schedule (Seconds (i), &ModifyLinkRate, &d10d11, DataRate(std::to_string(rateVal0->GetValue())+"Mbps"),Time::FromInteger(delayVal0->GetValue(), Time::MS));
        Simulator::Schedule (Seconds (i), &ModifyLinkRate, &d12d13, DataRate(std::to_string(rateVal0->GetValue())+"Mbps"),Time::FromInteger(delayVal0->GetValue(), Time::MS));
        Simulator::Schedule (Seconds (i), &ModifyLinkRate, &d14d15, DataRate(std::to_string(rateVal0->GetValue())+"Mbps"),Time::FromInteger(delayVal0->GetValue(), Time::MS));
        Simulator::Schedule (Seconds (i), &ModifyLinkRate, &d16d17, DataRate(std::to_string(rateVal0->GetValue())+"Mbps"),Time::FromInteger(delayVal0->GetValue(), Time::MS));
    }
    ...
    for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
        ...
        if (i->first == 1 || (i->first >= 3 && i->first <= 9)){
