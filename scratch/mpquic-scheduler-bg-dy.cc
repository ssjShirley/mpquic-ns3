...
//  LogComponentEnable ("PacketMetadata", log_precision);
    ...
    // for (int i=1; i < simulationEndTime; i++) {
    //     if (i % 2 == 0){
    //         Simulator::Schedule (Seconds (i), &ModifyLinkRate, &d1d8, DataRate(std::to_string(5-1)+"Mbps"));
    //         std::cout<<"time: "<< Seconds (2*i) <<" path0 : rate "<<std::to_string(10)<<"Mbps"<<"\n";
    //         Simulator::Schedule (Seconds (i), &ModifyLinkRate, &d6d9, DataRate(std::to_string(50-5)+"Mbps"));
